...
public class WebOrderTests {
	...
	// // /*
	// // * Step 1. Navigate to loginpage Step 2. Assert that you are on loginpage
	// Step
	// // * 3. Login using valid credentials Step 4. Click on view all products Step
	// 5.
	// // * Verify following products are displayed: MyMoney FamilyAlbum ScreenSaver
	// // Step
	...
	@Test(description = "fill out application in order page", priority = 3)
	public void orderPage() throws InterruptedException {
		Thread.sleep(1000);
		// assertEquals(driver.getTitle(), "Web Orders");
		loginPage.login(userId, password);
		Thread.sleep(1000);
		orderPageFunctions = new OrderPage(driver);
		orderPageFunctions.orderPageLink.click();
		Thread.sleep(1000);
		orderPageFunctions.productSelection.click();
		Thread.sleep(1000);
		orderPageFunctions.quantity.sendKeys("15");
		Thread.sleep(1000);
		String expectedName = "Komil Juraev";
		orderPageFunctions.customerName.sendKeys(expectedName);
		Thread.sleep(1000);
		orderPageFunctions.street.sendKeys("somewhere court");
		Thread.sleep(1000);
		orderPageFunctions.city.sendKeys("Owings Mills");
		Thread.sleep(1000);
		orderPageFunctions.state.sendKeys("MD");
		Thread.sleep(1000);
		orderPageFunctions.zip.sendKeys("20036");
		Thread.sleep(1000);
		orderPageFunctions.visaCard.click();
		Thread.sleep(1000);
		orderPageFunctions.cardNumber.sendKeys("6502156987");
		Thread.sleep(1000);
		orderPageFunctions.experitionDate.sendKeys("01/2025");
		Thread.sleep(1000);
		orderPageFunctions.processButton.click();
		Thread.sleep(1000);
		orderPageFunctions.goToAllOrdersPage.click();
		Thread.sleep(1000);
		String actualName = orderPageFunctions.verifyOnAllOrdersPage.getText();
